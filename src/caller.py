...
class caller():
  ...
  def __call__(self, config_yaml, result_path, test_samples, target_app, num_of_frozen_layers, processor,  source_features, source_labels, rank):
    ...
    folder_name = "caller"
    list_of_classes = global_config["caller"]["list_of_classes"]
    list_of_class_args = global_config["caller"]["list_of_class_args"]
    ...
    for i in range(rank, rank+1):
      ...
      for module_name in list_of_classes:
        ...
        mse0, mape0 = obj(source_features, source_labels, x2, lb2, tar_x_test, tar_y_test, rank)#obj(source_features, source_labels, new_tar_x_test, tar_y_test, rank)
        rowMSE =[]
        rowMAPE = []
