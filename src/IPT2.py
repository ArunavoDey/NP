...
class IPT2():
  ...
  def __call__(self, config_yaml, result_path, test_samples, target_app, num_of_frozen_layers, processor,  source_features, source_labels, rank):
    ...
    for i in range(rank, rank+1):
      ...
      source_model = util.sourceModelLoader(pca.fit_transform(source_features), source_labels, False, num_of_frozen_layers, True, global_config, target_app, True)
      callback2 = util.EarlyStoppingAtMinLoss(patience=40, arg_loss="loss")
      transformator = optunatransformator_nc.Transformator(dim = 7,
              #source_features.shape[1], 
              numOfLayersE = use_case_specific_config["layers"] , neuronsE = use_case_specific_config["neurons"] , activationE= "relu")
      ae = optunatransformator_nc.Autoencoder(intermediate_dim = 7, original_dim1 = source_features.shape[1])
      ...
      for epch in range(5000):
        ...
        rowArr.append(mse0)
        """
      transformed = ae.getEncoded(tar_x_test)
